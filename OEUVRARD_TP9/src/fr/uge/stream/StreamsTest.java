...
public class StreamsTest {
	...
	public static Map<Character, Set<String>> mapByFirstCharacterSet(List<String> list){
		return list.stream().collect(Collectors.groupingBy(s -> s.charAt(0), Collectors.toSet()));
	}
	...
	public static List<String> words(String s){
		return Arrays.stream(s.split(" ")).filter(s1 -> !s1.isEmpty()).toList();
	}
	...
	public static Set<String> words(List<String> list){
		return list.stream().flatMap(s -> Arrays.stream(s.split(" "))).filter(s -> !s.isEmpty()).collect(Collectors.toSet());
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(mapByFirstCharacterSet(List.of("AB","A","BA","C")));
